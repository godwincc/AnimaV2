...
**`GameHub` method surface, current state (Sanctum/Collection, then Weaving, then Resource/Treasure, then Shop — four separate sessions):**
- Foundation: `GetRoster`, `GetLedger`, `StartDelve`, `GetDelveStatus`.
...
- **Combat/Reforge/Boss are still not ported to hub methods** — Reforge is deliberately deferred (0% map odds, unresolved open items: the Crest-slot bug, the Wisp-tier question, screen visual identity), Combat/Boss is real follow-up work (Phase 5).
...
3. Godot client: not started. **Server (Accounts/Auth/DB persistence): implemented across several sessions — see Server / Accounts / Auth above.** `SanctumRoster`/`PersistentLedger` have real per-account Postgres persistence via `Anima.Server`; `DelveRun` remains in-memory-only per connection (by design, see its own section). Sanctum/Collection/Anima-Profile/Weaving/Delve-traversal/Resource/Treasure/Shop are now ported onto `GameHub`. Remaining server work: port Combat/Boss-victory onto `GameHub` methods (Reforge stays deliberately deferred — 0% map odds, unresolved open items), wire the rest of the `AccountArtifactStatEntity` write path (win-count on Boss Victory specifically — discovery-on-pickup landed via `ClaimTreasureNode`/`BuyWaresArtifact`), add real test coverage, wire real email delivery for password reset. Anima Profile's Delve History is still tracked nowhere at all (needs Combat/Boss on the hub first).
