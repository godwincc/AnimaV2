...
**`GameHub` method surface, current state (Sanctum/Collection, then Weaving, then Resource/Treasure — three separate sessions):**
- Foundation: `GetRoster`, `GetLedger`, `StartDelve`, `GetDelveStatus`.
- Sanctum/Collection: `GetRoster` also returns each Anima's parts list and `InTeam` flag; `SetTeam` persists the active-team selection to `AccountEntity.TeamAnimaIdsJson`; `GetArtifactCollection` returns the full 12-Artifact catalog joined against per-account discovered/won-with-count stats (`Discovered` is real as of this session, see Artifact Cap above; `DelvesWonWith` still always 0).
...
- Delve traversal: `MoveToNode` (a real gap this session found — `DelveRun.TryMoveTo` existed with nothing calling it).
- Resource/Treasure: `CollectResourceNode`/`ClaimTreasureNode` (both mark the node cleared INSIDE the same call that grants the reward, so a client can't double-claim by calling again — see their own comments), `ConvertPendingEmberToWisp`/`AugmentPendingEmber` (resolve the pending-Ember queue).
- **Combat/Shop/Reforge/Boss are still not ported to hub methods** — that's real follow-up work, not an oversight.
...
3. Godot client: not started. **Server (Accounts/Auth/DB persistence): implemented across several sessions — see Server / Accounts / Auth above.** `SanctumRoster`/`PersistentLedger` have real per-account Postgres persistence via `Anima.Server`; `DelveRun` remains in-memory-only per connection (by design, see its own section). Sanctum/Collection/Anima-Profile/Weaving/Delve-traversal/Resource/Treasure are now ported onto `GameHub`. Remaining server work: port Combat/Shop/Reforge/Boss-victory onto `GameHub` methods, wire the rest of the `AccountArtifactStatEntity` write path (win-count on Boss Victory specifically — discovery-on-pickup landed this session via `ClaimTreasureNode`), add real test coverage, wire real email delivery for password reset. Anima Profile's Delve History is still tracked nowhere at all (needs Combat/Boss on the hub first).
